...
public class CrearBaseDatos_Prueba {
	...
	public void insertarDatos(String dbName,String dbParam) {
		...
		try {
		  ...
		  st.executeUpdate("INSERT INTO Faccion(faccion_id,nombre_faccion,lore) VALUES(2,'Vikingos','Los vikingos desaparecieron hace siglos,tras escapar de sus destrozadas tierras natales en pos de costas desconocidas')");
		  st.executeUpdate("INSERT INTO Faccion(faccion_id,nombre_faccion,lore) VALUES(3,'Samurais','Originarios de una tienda allende los mares,cuentan la historia de un emperador y una patria que desaparecieron entre el mar y el fuego.')");
	...
	public void realizarConsultas(String dbName,String dbParam) {
		...
		try {
		  ...
		  while (rs2.next()){
		    Integer pj_id = rs2.getInt("personaje_id");
		    String name_pj = rs2.getString("nombre_personaje");
		    Integer atack = rs2.getInt("Ataque");
		    Integer def = rs2.getInt("Defensa");
		    Integer faction_id2 = rs2.getInt("faccion_id");
